...
var sys = require('sys'),
    jerk = require('jerk'),
    Y = require('yui3').useSync('yql', 'gallery-torelativetime');
...
jerk(function(j) {
    ...
    j.watch_for(/^seen ([^\s]+)$/, function(message) {
        var cmd = mysql.query("select channel, who, utterance, stamp from chat where who=? order by stamp desc limit 1",
            [ message.match_data[1] ]);
        cmd.addListener('row', function(r) {
            message.say( to( message, 2 ) + ": " + r.who + " was last seen on " + r.channel + " saying " + r.utterance + " approximately " + Y.toRelativeTime(r.stamp));
        });
    });
